...
echo "building and pushing image spring-petclinic-buildstage"
pushd source\output\src
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/spring-petclinic-buildstage .
popd

echo "building and pushing image spring-petclinic"
pushd source\output\src
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/spring-petclinic .
popd
