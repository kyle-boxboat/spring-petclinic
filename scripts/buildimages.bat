...
echo "building image spring-petclinic-buildstage"
pushd source\output\src
%CONTAINER_RUNTIME% build -f Dockerfile.buildstage -t spring-petclinic-buildstage .
popd

echo "building image spring-petclinic"
pushd source\output\src
%CONTAINER_RUNTIME% build -f Dockerfile -t spring-petclinic .
popd
